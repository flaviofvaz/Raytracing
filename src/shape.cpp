...
bool Box::intersect(const Ray& ray, Hit* hit) const
{
    ...
    // CRITICAL FIX: Ensure the intersection is valid and in front of the ray.
    if (t_final > EPSILON) 
    {
        hit->t = t_final;
        ...
        glm::vec3 calculated_outward_normal(0.0f);
        const float norm_epsilon = 1e-4f; // Tolerance for comparing point to face

        // Determine normal (ensure this logic correctly gives outward normal)
        // This selection logic might need refinement for points exactly on edges/corners,
        // but for points squarely on a face, it should work.
        if(std::abs(p.x - bMin.x) < norm_epsilon)
        { 
            calculated_outward_normal = glm::vec3(-1.0f, 0.0f, 0.0f);
        }
        ...
        {
            calculated_outward_normal = glm::vec3(0.0f, -1.0f, 0.0f);
        }
        ...
        {
            calculated_outward_normal = glm::vec3(0.0f, 0.0f, -1.0f);
        }
        ...
        {
             // Fallback: if p is not precisely on any face due to float issues,
             // determine dominant axis of (p - box_center) relative to box dimensions.
             // For simplicity here, we assume one of the above will be true for a valid intersection.
             // A more robust method might involve finding the face whose plane is closest to p.
        }

        // Apply the consistent normal and backface setup discussed previously:
        glm::vec3 geometric_face_normal = glm::normalize(calculated_outward_normal); // Ensure normalized if not already unit

        if (ray_starts_inside) 
        {
            hit->backface = true; 
            hit->normal = -geometric_face_normal;
        } 
        else 
        {
            hit->backface = false; 
